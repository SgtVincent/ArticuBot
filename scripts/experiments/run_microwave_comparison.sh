...
# Compare heuristic vs integrated_inverse demo generation for 10 valid demos.
# Outputs:
#  - logs: outputs/compare_sampling_logs/
#  - results: outputs/compare_sampling_results/microwave_7167_compare_10.json
...
record_result() {
  ...
  # Check if exp_dir exists before counting
  local valid=0
  local total=0
  if [[ -d "$exp_dir" ]]; then
      local counts
...
# Clean up previous experiments to start fresh
rm -rf "$ROOT_DIR/${SIM_DATASET_DIR}/${OBJECT_TYPE}_${OBJECT_ID}/experiment/${integrated_exp}"
rm -rf "$ROOT_DIR/${SIM_DATASET_DIR}/${OBJECT_TYPE}_${OBJECT_ID}/experiment/${heuristic_exp}"
rm -f "$integrated_log" "$heuristic_log"
rm -f "$summary_json"

run_method "integrated_inverse" "$integrated_exp" "$integrated_log" "--rm4d-map $RM4D_MAP --top-k-grasps 5 --object-z-offset 0.3 0.7"
run_method "heuristic" "$heuristic_exp" "$heuristic_log" "--top-k-grasps 5 --object-z-offset 0.3 0.7"
