...
## Example: Full Pipeline (using included sim_microwave_good)

The repository includes a ready-to-use example asset at `data/custom_objects/sim_microwave_good/`. You can run the full pipeline directly:

```bash
# 1. Activate environment
source prepare.sh

# 2. (Optional) Start GraspGen Docker for on-demand grasp generation
# Skip this if predicted_grasps.yml already exists in the asset folder
cd GraspGen/docker && ./run.sh ../.. --models ../../GraspGenModels

# 3. Generate demonstrations (from ArticuBot root)
python manipulation/gen_demo_custom.py \
    --asset-dir data/custom_objects/sim_microwave_good \
    --exp-name sim_microwave_demos \
    --num-to-generate 50 \
...
# 4. Check results
ls data/custom_objects/sim_microwave_good/experiment/sim_microwave_demos/
```
...
|-------------|--------------|---------|
| `base_config.yaml` | `generate_base_config_from_urdf_custom.py` | Object properties, paths, joint info |
| `mobility_v2.json` | `generate_base_config_from_urdf_custom.py` | Kinematic structure from URDF |
| `parts_render/*.npy` | `generate_base_config_from_urdf_custom.py` | Handle point cloud for RL observations |
| `predicted_grasps.yml` | GraspGen (pre-generated) | Grasp poses for default joint state |
| `configs/config_*.yaml` | `gen_demo_custom.py` | Randomized object configurations |
| `experiment/*/demo_*.pkl` | `gen_demo_custom.py` | Saved demonstration trajectories |
| `_tmp_graspgen_*/` | `gen_demo_custom.py` (on-demand) | Temporary URDF snapshots for grasp prediction |
